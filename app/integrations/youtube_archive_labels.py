...
def restore_setlist_credits() -> int:
    """Recover explicitly credited artists from the saved source setlists."""
    with get_connection() as conn:
        archives = conn.execute("SELECT id, setlist FROM youtube_live_archives").fetchall()
        performances = conn.execute("SELECT id, archive_id, timestamp_text, song_title FROM youtube_song_performances WHERE NULLIF(TRIM(original_artist), '') IS NULL").fetchall()
        credits = {}
...
async def refresh_archive_labels(only_source: str | None = None) -> dict[str, int]:
    print(f"Restored {restore_setlist_credits()} original artist credits", flush=True)
    ...
    for source, target, translate in (
        ...
        with get_connection() as conn:
            ...
            rows = conn.execute(f"""SELECT p.{source} AS original, COUNT(*) AS uses
                ...
                GROUP BY p.{source} ORDER BY uses DESC""").fetchall()
            conn.commit()
        ...
        with get_connection() as conn:
            ...
            if cached_updates:
                conn.execute(f"""UPDATE youtube_song_performances p SET {target} = known.label
                    ...
                    WHERE p.{source} = known.original AND NULLIF(TRIM(p.{target}), '') IS NULL""",
                    ([original for original, _ in cached_updates], [label for _, label in cached_updates]))
            conn.commit()
