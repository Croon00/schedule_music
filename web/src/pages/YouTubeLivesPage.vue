...
watch(artists, (rows) => {
  if (selectedArtistId.value === null && rows.length) selectedArtistId.value = rows[0].id
}, { immediate: true })
...
const addLive = useMutation({
  ...
  onSuccess: async (archive) => {
    ...
    if (matched) selectedArtistId.value = matched.id
...
<template>
  <div class="page youtube-page">
    ...
    <section class="artist-section">
      ...
      <div v-if="artistsQuery.isPending.value" class="artist-selector"><i v-for="n in 5" :key="n" class="artist-select-card loading" /></div>
      <div v-else-if="artists.length" class="artist-selector">
    ...
    <div v-if="archives.isPending.value" class="archive-grid"><i v-for="n in 6" :key="n" class="archive-card loading" /></div>
    <div v-else-if="archives.isError.value" class="alert alert--error">우타와꾸 기록을 불러오지 못했습니다.</div>
    <div v-else-if="!archives.data.value?.length" class="panel empty-state"><span>♫</span><strong>저장된 우타와꾸가 없습니다</strong><p>오른쪽 아래 등록 버튼으로 첫 방송을 추가할 수 있습니다.</p></div>
    <div v-else :class="viewMode === 'grid' ? 'archive-grid' : 'archive-list'">
    ...
    <details class="panel performance-search-panel">
    ...
    <button class="floating-register" aria-label="우타와꾸 등록" @click="openRegistration"><span>＋</span> 우타와꾸 등록</button>
    ...
    <AppModal :open="registrationOpen" title="우타와꾸 등록" description="아티스트와 YouTube URL을 입력하면 방송 정보와 댓글 셋리스트를 저장합니다." @close="registrationOpen = false">
      <form class="form-grid" @submit.prevent="addLive.mutate()">
        <label class="form-grid__wide">아티스트 이름
          ...
          <datalist id="registered-artists"><option v-for="artist in artists" :key="artist.id" :value="artist.name">{{ artist.display_name || artist.name }}</option></datalist>
        </label>
